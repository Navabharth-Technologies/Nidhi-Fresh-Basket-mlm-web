...
import { User, Calendar, CheckCircle, XCircle, Clock, Package, LogOut, Landmark, Check, X, Wallet, FileText, TrendingUp, ChevronLeft, ChevronRight, ChevronDown, ChevronUp } from 'lucide-react-native';
    ...
    const fetchAllData = useCallback(async () => {
        try {
            ...
            if (activeSection === 'withdraw') {
                ...
                const res = await apiClient.get('/withdraw/admin/requests', { params });
                setRequests(res.data);
            } else {
                const res = await apiClient.get('/kyc/admin/kyc-requests');
                const filtered = res.data.filter(item => {
                    const count = item.existing_packages_count || 0;
                    if (activeSection === 'purchase') return count === 0; // First time
                    if (activeSection === 'repurchase') return count >= 1; // Repeat purchase
                    return true;
                });
        ...
        <ScreenBackground admin>
            ...
            <View style={styles.container}>
                <View style={styles.dashboardGrid}>
                    ...
                    {renderDashboardCard('repurchase', 'Package Re-Activation', stats.pending_repurchases, TrendingUp, '#1e40af')}
...
const styles = StyleSheet.create({
    ...
    dashboardCard: { backgroundColor: 'rgba(255, 255, 255, 0.8)', width: '31%', borderRadius: 16, padding: 15, elevation: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 2, position: 'relative', overflow: 'hidden', borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.3)' },
    ...
    card: { backgroundColor: 'rgba(255, 255, 255, 0.85)', borderRadius: 16, padding: 16, marginBottom: 15, elevation: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 4, borderWidth: 1, borderColor: 'rgba(255, 255, 255, 0.3)' },
    ...
    userId: { fontSize: 12, color: '#6b7280', marginTop: 1 },
    ...
    amountText: { fontSize: 18, fontWeight: '800', color: '#1a531b' },
    ...
    walletLabel: { fontSize: 10, color: '#64748b', textTransform: 'uppercase' },
    walletValue: { fontSize: 12, fontWeight: 'bold', color: '#334155' },
    ...
    // Export & Filter UI
    exportSection: { backgroundColor: 'rgba(255, 255, 255, 0.95)', marginHorizontal: 15, marginBottom: 15, borderRadius: 16, padding: 12, borderWidth: 1, borderColor: '#eee', elevation: 2, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.05, shadowRadius: 5 },
